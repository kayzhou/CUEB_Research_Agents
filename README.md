...
EconTriad财经科研智能体：面向财经论文生产的模块化工作流，共享三角色内核（Lead Author · Researcher · Referee）与本地工具链约定，按研究类型分目录维护。
...
细节、启动步骤、目录约定与底线规则见各目录内的 `README.md`、`ORCHESTRATOR.md` 与 `使用手册.md`。

---

## 共同设计

- **三角色**：Lead Author（人类裁决）· Researcher（执行）· Referee（只攻击、不代笔）。
...
- 已有或即将产生**样本与识别/定价策略**的实证论文 → 进入 `Empirical-Economics-Agent/`。
- 核心是**模型设定、QML、渐近证明与 Monte Carlo**的理论计量论文 → 进入 `Theoretical-Econometrics-Agent/`。
